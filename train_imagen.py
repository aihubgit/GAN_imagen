...
def main():
    ...
    parser.add_argument('--sample_steps', default=64, type=int)  #(64, 32)
    parser.add_argument('--num_unets', default=2, type=int, help="additional unet networks")  # 1-unets=2, 2-unets=3
    parser.add_argument('--vocab_limit', default=None, type=int)
    parser.add_argument('--epochs', default=300, type=int) # 100
    ...
    parser.add_argument('--dim_mults', default="(1,2,3,4)", type=tuple_type)  #unet2 dim_milts (1,2,4,8)
    parser.add_argument("--start_size", default=64, type=int)
    parser.add_argument("--sample_unet", default=None, type=int)
    parser.add_argument('--device', type=str, default="cuda")
    parser.add_argument('--text_encoder', type=str, default="KETI-AIR/ke-t5-small") # t5-small
    parser.add_argument("--cond_scale", default=1.0, type=float, help="sampling conditional scale 0-10.0") # 5?
    ...
    parser.add_argument("--sigma_max", default=80, type=float)  # 80, 40 o, 640 x
    ...
    parser.add_argument('--samples_out', default="images")
    parser.add_argument('--train', action='store_true')
    parser.add_argument('--train_encoder', action='store_true')
    parser.add_argument('--shuffle_tags', action='store_true')
    parser.add_argument('--train_unet', type=int, default=2) #1
    ...
    parser.add_argument('--pretrained', default="KETI-AIR/ke-t5-small")   #  t5-small
...
def get_imagen(args, unet_dims=None, unet2_dims=None):
    ...
    image_sizes = (64, 256, 512)#get_image_sizes(args)
    ...
    if not args.no_elu:
        ...
        imagen = ImagenConfig(
            unets=unets,
            text_encoder_name=args.text_encoder,
            beta_schedules=["linear", "linear", "linear"],
            pred_objectives=["noise", "x_start", "x_start"],
            image_sizes=image_sizes,
            per_sample_random_aug_noise_level=True,
...
def make_training_samples(cond_images, styles, trainer, args, epoch, step, epoch_loss):
    ...
    sample_images = trainer.sample(texts=sample_texts,
                                   text_embeds=text_embeds,
                                   cond_images=sample_cond_images,  #sample_poses
                                   cond_scale=args.cond_scale,
...
def train(args):
    ...
    trainer = ImagenTrainer(imagen, precision=precision, lr=args.lr, use_ema=True)
        ...
        with tqdm(dl, unit="batches", disable=not trainer.accelerator.is_local_main_process) as tepoch:
            for data in tepoch:
                ...
                if step % args.sample_rate == 0:
                    if not args.no_patching:  #no_sample
