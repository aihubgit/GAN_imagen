...
DEFAULT_T5_NAME = 'KETI-AIR/ke-t5-base'  #google/t5-v1_1-base
...
def get_tokenizer(name):
    tokenizer = T5Tokenizer.from_pretrained(name, model_max_length=MAX_LENGTH, use_auth_token="true")
    return tokenizer

def get_model(name):
    model = T5EncoderModel.from_pretrained(name, use_auth_token="true")
    return model
...
def get_encoded_dim(name):
    if name not in T5_CONFIGS:
        ...
        config = T5Config.from_pretrained(name, use_auth_token="true")
